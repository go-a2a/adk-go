...
func (s *InMemoryMemoryService) AddSessionToMemory(ctx context.Context, session *session.Session) error {
	if session == nil {
...
func (s *InMemoryMemoryService) SearchMemory(ctx context.Context, appName, userID, query string) (*SearchMemoryResponse, error) {
	...
	for key, events := range s.sessionEvents {
		...
		for _, e := range events {
			...
			contentLower := strings.ToLower(e.Content.Parts[0].Text) // contentLower := strings.ToLower(e.Content)
