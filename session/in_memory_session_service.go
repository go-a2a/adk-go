...
func (s *InMemorySessionService) CreateSession(ctx context.Context, appName, userID, sessionID string) (*Session, error) {
	...
	// Create a new ses
	ses := NewSession(sessionID, appName, userID)

	// Initialize app state
	if _, ok := s.appState[appName]; ok {
		for k, v := range s.appState[appName] {
			ses.State.Set(AppPrefix+k, v)
		}
	} else {
...
func (s *InMemorySessionService) ListSessions(ctx context.Context, appName, userID string) (*ListSessionsResponse, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	emptyResponse := &ListSessionsResponse{}
...
func (s *InMemorySessionService) AppendEvent(ctx context.Context, appName, userID, sessionID string, ev *event.Event) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	updateSessionState(session, ev)

	// TODO(zchee): fix logic
	// Store any app-level state changes
	stateAction := ev.Actions
	if ok {
		for k, v := range stateAction.StateDelta {
...
func copySession(s *Session) *Session {
	...
	// Copy events
	copySession.Events = make([]*event.Event, len(s.Events))
	for i, e := range s.Events {
		copySession.Events[i] = e
	}

	// Copy state
	if s.State != nil {
		for k, v := range s.State.ToMap() {
			copySession.State.Set(k, v)
		}
	}
