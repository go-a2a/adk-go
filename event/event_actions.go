...
type EventActions struct {
	...
	// Ascalate is the agent is escalating to a higher level agent.
	Ascalate bool

	// RequestedAuthConfigs authentication configurations requested by tool responses.
	RequestedAuthConfigs map[string]any // map[string]AuthConfig
}
...
func NewEventActions() *EventActions {
	return &EventActions{
		...
		RequestedAuthConfigs: make(map[string]any), // *auth.AuthConfig
	}
}
